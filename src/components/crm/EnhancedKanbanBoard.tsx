...
export function EnhancedKanbanBoard({ clients, type }: EnhancedKanbanBoardProps) {
  ...
  const { data: statusConfigs = [] } = useStatusConfigs();
  const navigate = useNavigate();
  const [isDragging, setIsDragging] = useState(false);

  // Use dynamic configs from database, fallback to static if empty
    ...
    if (type === 'status') {
      if (statusConfigs.length > 0) {
        return statusConfigs.map(s => ({
          value: s.value,
          label: s.name,
          color: `bg-[${s.color}]/20 text-[${s.color}] border-[${s.color}]/30`,
        }));
      }
  ...
  }, [type, pipelineStages, statusConfigs]);
  ...
  // Get color for dynamic columns
  const getColumnColor = (column: ColumnConfig, isDynamic: boolean) => {
    if (!isDynamic) return column.color;
    // For dynamic configs, extract hex color and create tailwind-like classes
    const hexColor = (type === 'status' ? statusConfigs : pipelineStages)
      .find(c => c.value === column.value)?.color || '#3b82f6';
    return `bg-opacity-20 border-opacity-30`;
  };

  const getColumnStyle = (column: ColumnConfig) => {
    const config = (type === 'status' ? statusConfigs : pipelineStages)
      .find(c => c.value === column.value);
    if (config?.color) {
      return {
