...
import { useRoleBasedTasks } from '@/hooks/useRoleBasedData';
...
export default function EmployeeTasksPage() {
  ...
  const { data: taskData, refetch } = useRoleBasedTasks();
  const myTasks = taskData?.myTasks || [];
  const teamTasks = taskData?.teamTasks || [];
  ...
  }, [currentTasks, searchTerm, priorityFilter, projectFilter, assigneeFilter, dateRange]);
                ...
                <div key={stage.id} className="flex flex-col">
                  ...
                  <Droppable droppableId={stage.id}>
                    {(provided, snapshot) => (
                      <ScrollArea 
                        className={cn(
                          'flex-1 min-h-[300px] max-h-[calc(100vh-500px)] rounded-xl p-2 transition-all duration-200',
                                  ...
                                  <CardContent className="p-3">
                                    <div className="flex items-start gap-2">
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        {activeTab === 'team-tasks' && task.assigned_to && (
                                          <div className="flex items-center gap-2 mt-2 pt-2 border-t">
                                            <Avatar className="h-5 w-5">
                                              <AvatarFallback className="text-[10px]">
                                                {getAssigneeName(task.assigned_to).substring(0, 2).toUpperCase()}
                                              </AvatarFallback>
                                            </Avatar>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text">
              {isTeamLead ? 'Tasks Management' : 'My Tasks'}
            </h1>
            <p className="text-muted-foreground mt-1">
              {isTeamLead ? 'Manage your tasks and team tasks' : 'Manage and track your assigned tasks'}
            </p>
          </div>
          <div className="flex items-center gap-2">
